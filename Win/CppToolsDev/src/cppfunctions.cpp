...
void vector2str(std::vector<unsigned int> theVec, char* str)
{
...
void list2str(std::list<unsigned int> theList, char* str)
{
	str[0] = '\0';
	char buffer[CHAR_BUFFER] = "\0";
	for (std::list<unsigned int>::iterator it = theList.begin(); it != theList.end();)
	{
		sprintf(buffer, "%d", *it);
		strcat(str, buffer);
		it++;
...
void charvet2type(const char* vetc, std::list<unsigned int>& theList)
{
...
void charvet2type(const char* vetc, std::vector<unsigned int>& theVector)
{
	std::list<unsigned int> theList;
	charvet2type(vetc, theList);
	std::copy(theList.begin(), theList.end(), std::back_inserter(theVector));
